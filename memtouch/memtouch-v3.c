...
void *alloc_scratch_space(size_t memsize, const char *mempath, bool fill_random)
{
	/* allocate scratch space for memtouch */
	void *membuf = NULL;

	if (mempath) {
		int memfd = open(mempath, O_RDWR | O_CREAT, 0600);
		if (memfd < 0) {
			fprintf(stderr, "open %s", mempath);
			exit(EXIT_FAILURE);
		}

		int ret = ftruncate(memfd, memsize);
		if (ret < 0) {
			perror("ftruncate");
			exit(EXIT_FAILURE);
		}

		membuf = mmap(NULL, memsize, PROT_READ|PROT_WRITE, MAP_SHARED, memfd, 0);
		...
		close(memfd);
		fprintf(stderr, "scratch space %zu mbytes (path %s)\n", memsize / 1024 / 1024, mempath);
		...
		fprintf(stderr, "scratch space %zu mbytes\n", memsize / 1024 / 1024);
	}

	if (!fill_random) {
		fprintf(stderr, "skip filling scratch space with random data\n");
		return membuf;
	}

	/* read 1-page random data to buffer */
	...
	/* copy random data to scratch space */
	...
	fprintf(stderr, "fill scratch space with random data\n");

	/* if you want to make the content of mempath persistent, do msync() */
...
struct option longopts[] = {
	...
	{"cpu-bound", required_argument, NULL, 'b'},
	{"no-fill-random", no_argument, NULL, 'N'},
	{"mempath", required_argument, NULL, 'p'},
	{NULL, 0, NULL, 0},
};
...
{
	fprintf(stderr, "Memtouch (version $Id: memtouch-with-busyloop3.c 3632 2013-07-09 15:23:35Z takahiro $)\n");
