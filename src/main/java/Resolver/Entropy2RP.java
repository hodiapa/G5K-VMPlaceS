...
//		VJob v = new BasicVJob("v1");//Entropy2.0
		...
		if(reconfigurationPlan != null){
            ...
            Log.info("The following reconfiguration Plan will be performed:\n"+reconfigurationPlan);
		}
	...
	public void applyReconfigurationPlan() {
		if(reconfigurationPlan != null && !reconfigurationPlan.getActions().isEmpty()){
            ...
            try {
				File file = new File("logs/entropy/reconfigurationplan/" + this.loopID + "-"+ System.currentTimeMillis() + ".txt");
				file.getParentFile().mkdirs();
				PrintWriter pw = new PrintWriter(new BufferedWriter(new FileWriter(file)));
	...
	//Apply the reconfiguration plan logically (i.e. create/delete Java objects)
		private void applyReconfigurationPlanLogically(LinkedList<Action> sortedActions) throws InterruptedException{
			...
			for (Action a : sortedActions) {
			    ...
			    if (revDependencies.containsKey(a)) {
				    //Get the associated dependencies and update it
            ...
            int i = 20000;
            while(CentralizedResolverThread.ongoingMigration()){
                try {
                    Thread.sleep((long)(5));
                    i--;
                    if (i<0) {
                        System.out.println("There is an issue (ongoing migration that never returns :( : EXIT");
                        System.exit(-1);
                    }
            ...
            private void instantiateAndStart(Action a) throws InterruptedException{
                if(a instanceof Migration){
                    ...
                    CentralizedResolverThread.incMig();
                    CentralizedResolverThread.relocateVM(migration.getVirtualMachine(), migration.getHost().getName(),
                                                                    migration.getDestination().getName());
